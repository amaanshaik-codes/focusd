...
fn test_encrypt_decrypt_roundtrip() {
    // use crate-level helpers (pub(crate)) via crate root
    let master = "test-master";
    let api_key = "sk-test-123";
    let enc = crate::encrypt_api_key(api_key, master);
    let dec = crate::decrypt_api_key(&enc, master).expect("decrypt failed");
...
fn test_parse_chatgpt_response_variants() {
    ...
    let out = crate::backend::ai_provider::parse_chatgpt_response(&j);
    assert_eq!(out, "Hello from chatgpt");

    // variant with choices[0].text
    let j2 = json!({ "choices": [ { "text": "Legacy text" } ] });
    let out2 = crate::backend::ai_provider::parse_chatgpt_response(&j2);
...
fn test_parse_gemini_response() {
    ...
    let out = crate::backend::ai_provider::parse_gemini_response(&j);
